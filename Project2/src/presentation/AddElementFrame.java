...
import command.element.ElementCreateCommand;
import model.DomainModelException;

public class AddElementFrame extends JFrame {
		...
		add.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					dispose();
					try {
						new ElementCreateCommand(name, inventory, atmNum, atmMass).execute();
					} catch (DomainModelException e1) {
						e1.printStackTrace();
					}
				} catch (NumberFormatException e1) {
					new FailureFrame("Failed to create Element");
				}
